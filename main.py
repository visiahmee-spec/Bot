import telebot
import requests
import json,time
import re
import random
from telebot import types

# تخزين مؤقت للبيانات
user_data_storage = {}

# =========================
# إعدادات البوت
# =========================
TOKEN = '7497151281:AAHQLlqAR3dMcosj3uevoK3_E6PdHnM3inw'
bot = telebot.TeleBot(TOKEN)
# =========================
# دالة معلومات BIN
# =========================
def get_bin_info(bin_number):
    """الحصول على معلومات BIN من مصدرين"""
    try:
        response = requests.get(f"https://bins.antipublic.cc/bins/{bin_number[:6]}", timeout=5)
        if response.status_code == 200:
            bin_data = response.json()
            brand = bin_data.get("brand", "Unknown").upper()
            card_type = bin_data.get("type", "Unknown").upper()
            level = bin_data.get("level", "Unknown").upper()
            bank = bin_data.get("bank", "Unknown")
            country_name = bin_data.get("country_name", "Unknown")
            country_flag = bin_data.get("country_flag", "🏳️")
            return {
                'brand': brand,
                'card_type': card_type,
                'level': level,
                'bank': bank,
                'country_name': country_name,
                'country_flag': country_flag
            }
    except:
        pass
    
    try:
        response = requests.get(f'https://lookup.binlist.net/{bin_number[:6]}', timeout=5)
        if response.status_code == 200:
            data = response.json()
            return {
                'brand': data.get('scheme', 'Unknown').upper(),
                'card_type': data.get('type', 'Unknown').upper(),
                'level': "",
                'bank': data.get('bank', {}).get('name', 'Unknown'),
                'country_name': data.get('country', {}).get('name', 'Unknown'),
                'country_flag': data.get('country', {}).get('emoji', '🏳️')
            }
    except:
        pass
    
    return {
        'brand': "Unknown",
        'card_type': "Unknown",
        'level': "",
        'bank': "Unknown Bank",
        'country_name': "Unknown",
        'country_flag': "🏳️"
    }

# =========================
# رسالة الترحيب
# =========================
@bot.message_handler(commands=['start'])
def start_bot(message):
    chat_id = message.chat.id
    username = message.from_user.username
    first_name = message.from_user.first_name or "Guest"
    user_display = f"@{username}" if username else first_name
    a='''⠀⠀⠀⢠⣶⣿⣿⣿⠟⣴⣾⡿⢷⣾⣿⣿⣿⢯⣫⢶⣩⣾⡷⢿⣾⣿⠇⣿⣿⣿⢶⡇⣈⠖⣿⣶⡄⠀⠀⠀
⠀⠀⣠⣿⣿⣿⣿⣣⣾⣿⣟⣵⣿⣿⡿⣻⣶⢏⣴⣿⣿⢏⣵⣿⣿⢋⢸⣿⣿⢗⣿⢳⢥⣿⡜⢻⣿⣦⠀⠀
⠀⢴⣿⣿⣿⡯⣱⣿⣿⢏⣾⣿⡿⣯⣾⣟⣵⣿⡟⢟⣴⣿⣿⣛⣕⣫⢸⣿⡟⣽⡗⢺⣧⢿⣿⡎⣿⣿⣷⡀
⣸⣿⣿⣿⣛⣲⣿⣿⣡⣿⣿⣟⣵⣿⢭⠾⢋⢀⣴⣿⡿⡿⢩⣢⡯⡝⣿⣿⢳⣿⢐⡺⣿⡾⣿⣿⠼⣿⣿⣗
⣿⣿⣿⢏⣢⣿⠿⣴⣿⡿⢧⣾⡯⠁⠃⢈⣴⣿⣿⡻⡋⣤⣤⣶⣾⢹⣿⢣⣿⢇⢞⡄⣿⣷⢻⣿⡆⣿⣿⡝
⣿⣿⡏⢢⣽⢯⡾⣿⣿⣣⣿⠟⢠⢑⣴⣿⣿⠻⠃⠠⡺⣿⣿⣿⣋⣿⢫⣿⢏⣾⣷⣏⢹⣿⡝⣿⡧⣿⣿⡇
⣿⣟⢎⢑⣵⡿⣼⡳⣳⣿⡟⢐⣔⣿⣟⠟⠅⢰⣆⠀⢉⣄⢝⠿⣼⢫⣿⣏⣾⣿⣿⣿⠊⣿⣯⢻⡇⣿⣿⡷
⢟⣠⡾⢟⡡⣼⠧⣽⣿⡛⣴⣿⠯⢋⢔⣽⣶⣌⠛⣁⡄⣺⡮⡽⣳⣿⢗⣾⡿⣿⣿⣷⣎⠘⣿⣼⠇⣿⣿⣿
⣩⢂⡆⢚⢥⢏⣾⣟⣴⡩⠛⢀⠀⠤⠦⡑⠊⠿⢹⢻⣟⡿⣽⣽⡿⣋⣼⣯⣛⡝⠿⢿⣿⡣⡈⡆⠃⣿⣿⡏
⢳⡫⢍⣳⣴⡿⣩⣾⠅⠀⡠⡲⢜⠢⡂⢌⡣⡃⢈⡁⣕⣽⡿⢟⣑⡿⣿⣏⠋⠉⠙⠃⠪⠓⢎⠷⢹⣿⣿⡇
⡚⠻⡴⣫⡕⣾⣿⡿⣰⣯⢶⡵⢦⡕⣬⢊⡕⣉⢌⣡⣾⡻⠃⢀⠾⡁⠻⣷⡘⠛⢂⣤⡂⠀⠠⠋⣾⣿⣿⡷
⣻⡻⠧⢸⡇⣿⣿⠇⣿⣿⣿⣾⣷⣿⣼⣣⣞⢔⡵⠟⡉⡠⡘⡃⢉⡘⠃⢙⢽⣻⣿⣿⡿⠂⠀⢠⣿⣿⣿⠠
⡥⠙⠷⣾⡇⣿⣿⢧⣿⢹⣿⣿⣿⡿⡿⡹⢕⢩⢴⣎⠶⡱⢎⠕⣡⠆⡐⢌⢄⡡⢨⢣⡴⠁⢀⣼⡗⣿⣏⣆
⠆⡰⢆⣿⠁⣿⣿⢹⣿⣿⣿⢽⣫⣥⣷⠁⠉⡛⠓⠲⠯⣞⡣⣛⢴⡣⢞⠤⠢⢌⢢⠫⠀⠀⣤⣿⢽⣿⢺⣙
⢐⢜⢸⡯⠀⣿⣿⢸⣿⣷⣾⣿⣿⣿⡇⠨⠌⣀⢋⡀⠡⠀⢝⣈⢾⡵⣮⠲⡕⢎⠔⡑⢀⣎⣸⡷⢾⡟⡇⣿
⡩⠂⣿⢇⠂⣿⣿⠈⢻⣿⣿⣿⣿⡿⢇⠐⢆⠶⡀⢈⠃⡱⠀⠤⣼⣿⣷⣟⣜⡧⣚⠈⣼⡇⣿⢳⣟⠻⣼⣿
⠠⣹⡟⠤⣛⢸⣿⢀⠀⠹⣿⠟⠫⣠⣶⣇⡣⣟⠜⡠⣛⠔⢡⢹⣿⣿⣿⣿⣿⣿⠇⢹⣿⢹⣟⣾⡟⣊⢿⣿
⢢⣿⢉⠶⡱⢺⣿⠨⢎⠵⡐⡱⣿⣿⣿⣿⠔⡯⢢⡕⣍⣌⣸⢸⣿⣿⣿⣿⠿⡊⣊⣿⡏⣼⣧⣿⢑⣯⣿⣏
⣿⢣⡧⣛⢜⣹⣿⠰⡣⣛⠄⡠⡈⠻⣻⠟⡊⠶⠱⡪⢪⣾⡌⡏⠿⠟⡛⢡⢪⠑⣸⣟⡁⣿⢸⠧⣼⣼⡏⣆
⢧⣿⡷⣭⢪⣹⡟⢨⡕⢭⢢⡕⢎⠴⡠⠈⢾⠋⠕⠕⡛⢅⡱⢄⠶⠱⠆⠤⡰⠎⣿⠧⢴⡿⢏⢰⣇⡿⢸⡗'''
    z='''⡀⠄⢀                            ⣿⣿⠄⠄⠄⢸⡇⠄⠄
⠄⠄⠄⣾⣿⠿⠿⠶⠿⢿⣿⣿⣿⣿⣦⣤⣄⢀⡅⢠⣾⣛⡉⠄⠄⠄⠸⢀⣿⠄
⠄⠄⢀⡋⣡⣴⣶⣶⡀⠄⠄⠙⢿⣿⣿⣿⣿⣿⣴⣿⣿⣿⢃⣤⣄⣀⣥⣿⣿⠄
⠄⠄⢸⣇⠻⣿⣿⣿⣧⣀⢀⣠⡌⢻⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠿⠿⣿⣿⣿⠄
⠄⢀⢸⣿⣷⣤⣤⣤⣬⣙⣛⢿⣿⣿⣿⣿⣿⣿⡿⣿⣿⡍⠄⠄⢀⣤⣄⠉⠋⣰
⠄⣼⣖⣿⣿⣿⣿⣿⣿⣿⣿⣿⢿⣿⣿⣿⣿⣿⢇⣿⣿⡷⠶⠶⢿⣿⣿⠇⢀⣤
⠘⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣽⣿⣿⣿⡇⣿⣿⣿⣿⣿⣿⣷⣶⣥⣴⣿⡗
⢀⠈⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡟⠄
⢸⣿⣦⣌⣛⣻⣿⣿⣧⠙⠛⠛⡭⠅⠒⠦⠭⣭⡻⣿⣿⣿⣿⣿⣿⣿⣿⡿⠃⠄
⠘⣿⣿⣿⣿⣿⣿⣿⣿⡆⠄⠄⠄⠄⠄⠄⠄⠄⠹⠈⢋⣽⣿⣿⣿⣿⣵⣾⠃⠄
⠄⠘⣿⣿⣿⣿⣿⣿⣿⣿⠄⣴⣿⣶⣄⠄⣴⣶⠄⢀⣾⣿⣿⣿⣿⣿⣿⠃⠄⠄
⠄⠄⠈⠻⣿⣿⣿⣿⣿⣿⡄⢻⣿⣿⣿⠄⣿⣿⡀⣾⣿⣿⣿⣿⣛⠛⠁⠄⠄⠄
⠄⠄⠄⠄⠈⠛⢿⣿⣿⣿⠁⠞⢿⣿⣿⡄⢿⣿⡇⣸⣿⣿⠿⠛⠁⠄⠄⠄⠄⠄
⠄⠄⠄⠄⠄⠄⠄⠉⠻⣿⣿⣾⣦⡙⠻⣷⣾⣿⠃⠿⠋⠁⠄⠄⠄⠄⠄⢀⣠⣴
⣿⣿⣿⣶⣶⣮⣥⣒⠲⢮⣝⡿⣿⣿⡆⣿⡿⠃⠄⠄⠄⠄⠄⠄⠄⣠⣴⣿⣿⣿'''
    x='''⣿⣿⣿⣿⣿⣿⣿⡇⡌⡰⢃⡿⡡⠟⣠⢹⡏⣦⢸⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⡿⢰⠋⡿⢋⣐⡈⣽⠟⢀⢻⢸⡂⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣋⠴⢋⡘⢰⣄⣀⣅⣡⠌⠛⠆⣿⡄⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣶⣁⣐⠄⠹⣟⠯⢿⣷⠾⠁⠥⠃⣹⣿⣿⣿⣿⣿
⣿⣿⣿⣿⠟⠋⡍⢴⣶⣶⣶⣤⣭⡐⢶⣾⣿⣶⡆⢨⠛⠻⣿⣿⣿
⣿⣿⣿⢏⣘⣚⣣⣾⣿⣿⣿⣿⣿⣿⢈⣿⣿⣿⣧⣘⠶⢂⠹⣿⣿
⣿⣿⠃⣾⣿⣿⣿⣿⣿⣿⡿⠿⠿⠿⡀⢿⣿⣿⣿⣿⣿⣿⡇⣿⣿
⣿⣿⡄⣿⣿⣿⣿⣿⣿⡯⠄⠄⠾⠿⠿⢦⣝⠻⣿⣿⣿⣿⠇⣿⣿
⣿⣿⣷⣜⠿⢿⣿⡿⠟⣴⣾⣿⡇⢰⣾⣦⡹⣷⣮⡙⢟⣩⣾⣿⣿
⣿⣿⣿⣿⣿⣆⢶⣶⣦⢻⣿⣿⣷⢸⣿⣿⣷⣌⠻⡷⣺⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⡜⢿⣿⡎⢿⣿⣿⡬⣿⣿⣿⡏⢦⣔⠻⣿⣿⣿⣿
⣿⣿⣿⣿⣿⣿⣿⠎⠻⣷⡈⢿⣿⡇⢛⣻⣿⣿⢸⣿⣷⠌⡛⢿⣿
⣿⣿⣿⣿⣿⣿⡏⢰⣷⡙⢷⣌⢻⣿⣿⣿⣿⣿⢸⡿⢡⣾⣿⡶⠻
⣿⣿⣿⣿⣿⡟⣰⣶⣭⣙⠊⣿⣷⣬⣛⠻⣿⣿⠈⣴⣿⣿⣿⠃⠄
⣿⣿⣿⣿⡟⠄⠹⢿⣿⣿⣿⣤⠻⠟⠋⠡⠘⠋⢸⣿⣿⡿⠁⠄⠄
⣿⣿⣿⣿⠁⠄⠄⠄⠙⢻⣿⣿⣇⠄⠄⠄⠄⠄⣺⡿⠛⠄⠄⠄⠄
⣿⣿⣿⡏⠄⠄⠄⠄⠄⠄⠄⠉⠻⠷⠄⢠⣄⠄⠋⠄⠄⠄⠄⠄⠄'''
    c='''⣿⣿⣿⣿⠛⠛⠉⠄⠁⠄⠄⠉⠛⢿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⡟⠁⠄⠄⠄⠄⠄⠄⠄⠄⠄⠄⣿⣿⣿⣿⣿⣿⣿
⣿⣿⡇⠄⠄⠄⠐⠄⠄⠄⠄⠄⠄⠄⠠⣿⣿⣿⣿⣿⣿
⣿⣿⡇⠄⢀⡀⠠⠃⡐⡀⠠⣶⠄⠄⢀⣿⣿⣿⣿⣿⣿
⣿⣿⣶⠄⠰⣤⣕⣿⣾⡇⠄⢛⠃⠄⢈⣿⣿⣿⣿⣿⣿
⣿⣿⣿⡇⢀⣻⠟⣻⣿⡇⠄⠧⠄⢀⣾⣿⣿⣿⣿⣿⣿
⣿⣿⣿⣟⢸⣻⣭⡙⢄⢀⠄⠄⠄⠈⢹⣯⣿⣿⣿⣿⣿
⣿⣿⣿⣭⣿⣿⣿⣧⢸⠄⠄⠄⠄⠄⠈⢸⣿⣿⣿⣿⣿
⣿⣿⣿⣼⣿⣿⣿⣽⠘⡄⠄⠄⠄⠄⢀⠸⣿⣿⣿⣿⣿
⡿⣿⣳⣿⣿⣿⣿⣿⠄⠓⠦⠤⠤⠤⠼⢸⣿⣿⣿⣿⣿
⡞⣸⣿⣿⢏⣼⣶⣶⣶⣶⣤⣶⡤⠐⣿⣿⣿⣿⣿⣿⣿
⣯⣽⣛⠅⣾⣿⣿⣿⣿⣿⡽⣿⣧⡸⢿⣿⣿⣿⣿⣿⣿
⣿⣿⣿⡷⠹⠛⠉⠁⠄⠄⠄⠄⠄⠄⠐⠛⠻⣿⣿⣿⣿
⣿⣿⣿⠃⠄⠄⠄⠄⠄⣠⣤⣤⣤⡄⢤⣤⣤⣤⡘⠻⣿
⣿⣿⡟⠄⠄⣀⣤⣶⣿⣿⣿⣿⣿⣿⣆⢻⣿⣿⣿⡎⠝
⣿⡏⠄⢀⣼⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡎⣿⣿⣿⣿⠐
⣿⡏⣲⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⢇⣿⣿⣿⡟⣼
⣿⡠⠜⣿⣿⣿⣿⣟⡛⠿⠿⠿⠿⠟⠃⠾⠿⢟⡋⢶⣿
⣿⣧⣄⠙⢿⣿⣿⣿⣿⣿⣷⣦⡀⢰⣾⣿⣿⡿⢣⣿⣿
⣿⣿⣿⠂⣷⣶⣬⣭⣭⣭⣭⣵⢰⣴⣤⣤⣶⡾⢐⣿⣿
⣿⣿⣿⣷⡘⣿⣿⣿⣿⣿⣿⣿⢸⣿⣿⣿⣿⢃⣼⣿⣿'''
    v='''⣿⣿⣿⣿⣿⣿⠟⠋⠁⣀⣤⡄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⢿⣿⣿
⣿⣿⣿⣿⠋⠁⠀⠀⠺⠿⢿⣿⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠘⠻⣿
⣿⣿⡟⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⣤⣤⣤⣤⠀⠀⠀⠀⠀⣤⣦⣄⠀⠀
⣿⡟⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣤⣶⣿⠏⣿⣿⣿⣿⣿⣁⠀⠀⠀⠛⠙⠛⠋⠀⠀
⡿⠀⠀⠀⠀⠀⠀⠀⠀⡀⠀⣰⣿⣿⣿⣿⡄⠘⣿⣿⣿⣿⣷⠄⠀⠀⠀⠀⠀⠀⠀⠀
⡇⠀⠀⠀⠀⠀⠀⠀⠸⠇⣼⣿⣿⣿⣿⣿⣷⣄⠘⢿⣿⣿⣿⣅⠀⠀⠀⠀⠀⠀⠀⠀
⠁⠀⠀⠀⣴⣿⠀⣐⣣⣸⣿⣿⣿⣿⣿⠟⠛⠛⠀⠌⠻⣿⣿⣿⡄⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⣶⣮⣽⣰⣿⡿⢿⣿⣿⣿⣿⣿⡀⢿⣤⠄⢠⣄⢹⣿⣿⣿⡆⠀⠀⠀⠀⠀⠀
⠀⠀⠀⣿⣿⣿⣿⣿⡘⣿⣿⣿⣿⣿⣿⠿⣶⣶⣾⣿⣿⡆⢻⣿⣿⠃⢠⠖⠛⣛⣷⠀
⠀⠀⢸⣿⣿⣿⣿⣿⣿⣾⣿⣿⣿⣿⣿⣿⣮⣝⡻⠿⠿⢃⣄⣭⡟⢀⡎⣰⡶⣪⣿⠀
⠀⠀⠘⣿⣿⣿⠟⣛⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣿⣿⣿⡿⢁⣾⣿⢿⣿⣿⠏⠀
⠀⠀⠀⣻⣿⡟⠘⠿⠿⠎⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣵⣿⣿⠧⣷⠟⠁⠀⠀
⡇⠀⠀⢹⣿⡧⠀⡀⠀⣀⠀⠹⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠋⢰⣿⠀⠀⠀⠀
⡇⠀⠀⠀⢻⢰⣿⣶⣿⡿⠿⢂⣿⣿⣿⣿⣿⣿⣿⢿⣻⣿⣿⣿⡏⠀⠀⠁⠀⠀⠀⠀
⣷⠀⠀⠀⠀⠈⠿⠟⣁⣴⣾⣿⣿⠿⠿⣛⣋⣥⣶⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀⠀⠀⠀ ⣿⡀'''
    b='''⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⠤⠤⠤⠤⠤⢄⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣴⠊⠉⠀⠀⠀⠀⠀⠀⠀⠀⠈⠑⠢⣀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣴⡿⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠳⡀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣾⠏⠀⠀⠀⠀⢀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢈⣆⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣼⡇⠀⣀⠀⢠⠀⡜⠀⠀⢀⡇⠀⠀⠀⠀⠀⠀⠀⠀⠀⣿⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⣸⣿⠇⠀⡃⢀⠀⣴⡇⠀⣸⣿⡅⡀⠀⠀⠀⠀⡆⠀⠀⠀⣿⡇⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⣿⣿⢠⢰⡇⡎⢠⣻⣄⣠⣿⠛⣿⣿⠀⠀⠀⢠⣿⠶⡄⠀⣿⡇⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⢹⡇⢸⣾⣇⣿⣿⠿⣿⣿⡿⠾⢿⣿⢷⡶⢶⡾⣿⣿⣷⣿⣾⣦⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣷⠈⣿⣿⣿⣿⠀⣿⣿⠃⠰⣾⣿⠀⠃⠸⠀⣿⡇⢰⠄⣿⡟⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣿⢀⣿⣿⣿⣿⠀⠻⣿⠀⠰⢿⣿⡇⢰⠀⢰⣿⠇⢸⠀⣿⡇⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣿⢸⣿⣿⣿⠛⠻⡿⠿⠿⠿⠿⠿⠿⠿⠷⠾⠿⣶⣶⣶⣿⡇⠀
⠀⠀⠀⠀⠀⠀⠀⠀⢀⣸⣿⣾⣿⣿⣿⡄⠀⣧⠀⠀⠀⠀⠀⠀⠀⠀⠀⢠⡇⢰⠇⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⣰⠋⠀⠀⠀⠈⠹⣿⣷⡀⢸⣄⡀⠀⢠⣤⠄⢀⣤⣾⣿⠀⣜⣀⣀⠀⠀
⠀⠀⠀⠀⠀⠀⡸⠁⠀⣀⡀⠀⠀⡀⠈⢿⣧⠀⣯⠙⠶⢦⣴⣾⣿⠿⢛⡇⢰⠃⠀⠀⠱⡀
⠀⠀⠀⠀⠀⠀⡇⠀⠀⠀⠀⠀⠀⣿⠀⠘⣿⣆⣿⢿⣦⣼⡂⠀⣠⠞⢹⣧⡏⠀⠀⠀⠀⢃
⠀⠀⠀⠀⠀⠀⡇⠀⠀⠀⠀⠀⠀⢿⡇⠀⢹⣿⣿⠑⠻⣏⠉⠟⠁⠀⣼⣿⣡⣿⠀⠀⠀⡟
⠀⠀⠀⠀⢀⡼⡃⠀⠀⠀⠀⠀⠀⢸⣿⡤⠄⣿⣿⠀⠀⠀⠀⢀⠀⠀⡟⢻⢿⡇⠀⠀⠀⡇
⠀⠀⠀⢀⣾⢿⣷⠀⠀⠀⠀⠀⠀⢸⣿⠀⠀⣿⣿⠀⠀⠀⠀⠸⡄⠀⢷⣾⠀⠙⢢⡀⠀⠃
⠀⠀⢠⣾⠏⣾⣿⠀⠀⠀⠀⠀⠀⢹⣿⠀⢀⣿⣿⠀⠀⠀⠀⠀⠈⢆⠘⣿⡆⠀⠀⠙⢴⠀
⠀⢀⡎⠊⣸⣿⣿⡇⠀⠀⠀⠀⠀⢸⣿⠀⣾⣿⠏⠀⠀⠀⠀⠀⠀⠈⢆⢸⡇⠀⠀⠀⠘⡇
⢀⠼⠇⠀⠿⠿⠿⠿⠀⠀⠀⠀⠀⠻⠿⠀⠉⠁⠀⠀⠀⠀⠀⠀⠀⠀⠼⠎⠁⠀⠀⠀⠠⢷'''
    n='''⣿⣿⣯⠉⠄⠄⠄⠄⠄⠄⡄⠄⠄⠄⠄⠄⠄⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⡟⠁⠄⠄⠄⠄⠄⢀⢀⠃⠄⠄⠄⠄⠄⠄⠘⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⡇⠄⠄⣾⣳⠄⠄⢀⣄⣦⣶⣴⠂⢒⠄⠄⠄⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
⣿⣿⡄⠄⠈⠚⡆⠄⢸⣿⣿⣿⣯⠋⡏⠄⠄⢸⣿⣿⣿⠿⠛⠛⠿⣿⣿⣿⣿⣿
⣿⣿⠟⣂⣀⣀⣀⡀⠠⠻⣷⣎⡼⠞⠓⠦⣤⣛⣋⣭⣴⣾⣿⣿⣷⣌⠻⣿⣿⣿
⣿⠋⣼⣿⣿⣿⣿⣿⣷⣦⣍⣙⠻⠳⠄⠄⠈⠙⠿⢿⣿⣿⣿⣿⣿⡟⣰⣿⣿⣿
⡟⢸⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣶⣄⣀⠄⠄⢀⣤⣤⣭⡛⠛⣩⣴⣿⣿⣿⣿
⣷⠸⠿⠛⠉⠙⠛⠿⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠟⠛⠷⠦⣹⣿⣿⣿⣿
⣿⣧⠄⠄⠄⢀⣴⣷⣶⣦⣬⣭⣉⣙⣛⠛⠿⠿⠿⠟⠁⡀⠄⠄⠄⢁⣿⣿⣿⣿
⣿⣿⡅⠄⢀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣦⣍⠲⣶⣤⣄⡀⠄⣴⣿⣿⣿⣿⣿
⣿⣿⣷⠄⣾⡏⢿⣿⣿⣿⣿⣿⣿⣿⠿⠿⠄⠹⣷⡌⢿⣿⣿⣷⣦⡙⢿⣿⣿⣿
⣿⣿⣿⣷⡌⢷⡘⣿⣿⣿⣿⣿⣿⣧⣀⣀⡀⠄⠈⠹⡈⣿⣿⣿⣿⣿⣦⡙⣿⣿
⣿⣿⣿⣿⣿⣎⢷⡘⢿⣿⣿⣿⣿⣿⣿⣿⠃⠄⣼⣶⡇⣿⣿⣿⣿⣿⣿⠓⠜⣿
⣿⣿⣿⣿⣿⣿⣎⢻⣦⡙⠿⣿⣿⣿⣿⣿⣿⣿⣿⠟⠄⣿⣿⣿⣿⣿⣿⣄⡀⢸
⣿⣿⣿⣿⣿⡿⢃⢼⣿⣿⣷⣤⣍⣉⣙⣛⣛⣉⣥⡄⠄⢿⣿⣿⣿⣿⡿⠟⣥⣿
⣿⣿⣿⡿⢋⣴⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣟⣿⣿⢁⣷⣤⣍⣉⣉⣭⣴⣾⣿⣿'''
    m='''⣿⠟⣽⣿⣿⣿⣿⣿⢣⠟⠋⡜⠄⢸⣿⣿⡟⣬⢁⠠⠁⣤⠄⢰⠄⠇⢻⢸
⢏⣾⣿⣿⣿⠿⣟⢁⡴⡀⡜⣠⣶⢸⣿⣿⢃⡇⠂⢁⣶⣦⣅⠈⠇⠄⢸⢸
⣹⣿⣿⣿⡗⣾⡟⡜⣵⠃⣴⣿⣿⢸⣿⣿⢸⠘⢰⣿⣿⣿⣿⡀⢱⠄⠨⢸
⣿⣿⣿⣿⡇⣿⢁⣾⣿⣾⣿⣿⣿⣿⣸⣿⡎⠐⠒⠚⠛⠛⠿⢧⠄⠄⢠⣼
⣿⣿⣿⣿⠃⠿⢸⡿⠭⠭⢽⣿⣿⣿⢂⣿⠃⣤⠄⠄⠄⠄⠄⠄⠄⠄⣿⡾
⣼⠏⣿⡏⠄⠄⢠⣤⣶⣶⣾⣿⣿⣟⣾⣾⣼⣿⠒⠄⠄⠄⡠⣴⡄⢠⣿⣵
⣳⠄⣿⠄⠄⢣⠸⣹⣿⡟⣻⣿⣿⣿⣿⣿⣿⡿⡻⡖⠦⢤⣔⣯⡅⣼⡿⣹
⡿⣼⢸⠄⠄⣷⣷⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣕⡜⡌⡝⡸⠙⣼⠟⢱⠏
⡇⣿⣧⡰⡄⣿⣿⣿⣿⡿⠿⠿⠿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣋⣪⣥⢠⠏⠄
⣧⢻⣿⣷⣧⢻⣿⣿⣿⡇⠄⢀⣀⣀⡙⣿⣿⣿⣿⣿⣿⣿⣿⣿⡇⠂⠄⠄
⢹⣼⣿⣿⣿⣧⡻⣿⣿⣇⣴⣿⣿⣿⣷⢸⣿⣿⣿⣿⣿⣿⣿⣿⣰⠄⠄⠄
⣼⡟⡟⣿⢸⣿⣿⣝⢿⣿⣾⣿⣿⣿⢟⣾⣿⣿⣿⣿⣿⣿⣿⣿⠟⠄⡀⡀
⣿⢰⣿⢹⢸⣿⣿⣿⣷⣝⢿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠿⠛⠉⠄⠄⣸⢰⡇
⣿⣾⣹⣏⢸⣿⣿⣿⣿⣿⣷⣍⡻⣛⣛⣛⡉⠁⠄⠄⠄⠄⠄⠄⢀⢇⡏⠄'''
    r=random.choice([a,x,z,m,c,v,n,b])
    # في دالة start_bot، عدل رسالة الترحيب:
    msg = f"""<b>𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼 𝗧𝗵𝗲 𝗖𝗿𝗲𝗱𝗶𝘁 𝗖𝗮𝗿𝗱𝘀 𝗕𝗼𝘁</b>
    <pre>{r}</pre>

<b>𝗖𝗵𝗮𝗻𝗻𝗲𝗹:</b> @A_A_T_T943
<b>𝗢𝘄𝗻𝗲𝗿:</b> @A_A_T_T   |  𝐴𝐻𝑀𝐸𝐷

<b>𝗔𝘃𝗮𝗶𝗹𝗮𝗯𝗹𝗲 𝗰𝗼𝗺𝗺𝗮𝗻𝗱𝘀:</b>

<b>/fake [𝗰𝗼𝘂𝗻𝘁𝗿𝘆]</b> - 𝗚𝗲𝗻𝗲𝗿𝗮𝘁𝗲 𝗳𝗮𝗸𝗲 𝗶𝗻𝗳𝗼
<b>/gen [𝗕𝗜𝗡|𝗠𝗼𝗻𝘁𝗵|𝗬𝗲𝗮𝗿|𝗖𝗩𝗩]</b> - 𝗚𝗲𝗻𝗲𝗿𝗮𝘁𝗲 𝗰𝗮𝗿𝗱𝘀
<b>/bin [𝗰𝗮𝗿𝗱 𝗻𝘂𝗺𝗯𝗲𝗿]</b> - 𝗖𝗵𝗲𝗰𝗸 𝗕𝗜𝗡 𝗶𝗻𝗳𝗼
<b>/url [𝘄𝗲𝗯𝘀𝗶𝘁𝗲_𝘂𝗿𝗹]</b> - 𝗔𝗻𝗮𝗹𝘆𝘇𝗲 𝘄𝗲𝗯𝘀𝗶𝘁𝗲
<b>/analyze_all</b> - 𝗔𝗻𝗮𝗹𝘆𝘇𝗲 𝗮𝗹𝗹 𝘂𝗿𝗹𝘀 𝗶𝗻 𝗳𝗶𝗹𝗲
<b>/𝗵𝗲𝗹𝗽</b> - 𝗙𝘂𝗹𝗹 𝗰𝗼𝗺𝗺𝗮𝗻𝗱𝘀 𝗹𝗶𝘀𝘁

# Upload .txt file with URLs for bulk analysis
#سيتم اضافه بوابات ودوركات وازرار مخصصه لكلشي سيرج مواقع حمايه البوت في مرحله التطوير
"""
    
    bot.send_message(chat_id, msg, parse_mode='HTML')
# =========================
# fake command - Fixed
# =========================
# =========================
# Fake Command
# =========================
@bot.message_handler(commands=['fake'])
def fake_command(message):
    try:
        if len(message.text.split()) > 1:
            code = message.text.split()[1].upper()
        else:
            code = 'US'
        
        valid_codes = ['US', 'UK', 'IS', 'JO', 'KSA', 'MO']
        if code not in valid_codes:
            code = 'US'
        
        response = requests.get(f'https://randomuser.me/api/?nat={code}', timeout=10)
        data = response.json()
        
        if 'results' not in data:
            bot.reply_to(message, "❌ Could not generate info. Try again.")
            return
        
        result = data['results'][0]
        
        name = f"{result['name']['title']} {result['name']['first']} {result['name']['last']}"
        street = f"{result['location']['street']['number']} {result['location']['street']['name']}"
        city = result['location']['city']
        state = result['location']['state']
        country = result['location']['country']
        postcode = str(result['location']['postcode'])
        phone = result['phone']
        email = result['email']
        
        msg = f"""
<strong>𝗙𝗮𝗸𝗲 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻 ✅
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐂𝐨𝐮𝐧𝐭𝐫𝐲: <code>{country}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐍𝐚𝐦𝐞: <code>{name}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐂𝐢𝐭𝐲: <code>{city}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐒𝐭𝐚𝐭𝐞: <code>{state}</code>
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐙𝐢𝐩 𝐂𝐨𝐝𝐞: <code>{postcode}</code>
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐒𝐭𝐫𝐞𝐞𝐭: <code>{street}</code>
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐏𝐡𝐨𝐧𝐞: <code>{phone}</code>
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐄𝐦𝐚𝐢𝐥: <code>{email}</code>
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">⌤</a>] 𝐀𝐧𝐚𝐥𝐲𝐳𝐞𝐝 𝐛𝐲: <a href='tg://user?id={message.from_user.id}'>𝐴𝐻𝑀𝐸𝐷</a>@A_A_T_T</strong>"""
        
        bot.reply_to(message, msg, parse_mode='HTML')
        
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {str(e)}")

@bot.message_handler(commands=['gen'])
def generate_card(message):
    try:
        if len(message.text.split()) < 2:
            bot.reply_to(message, "⚠️ Usage: /gen BIN|Month|Year|CVV\nExample: /gen 552742|12|25|123")
            return
        
        input_text = message.text.split('/gen ', 1)[1]
        parts = re.split(r'[|:/]', input_text)
        
        if len(parts) < 1:
            bot.reply_to(message, "❌ Invalid format. Use: BIN|Month|Year|CVV")
            return
        
        bin_part = re.sub(r'[^0-9]', '', parts[0])
        month = re.sub(r'[^0-9]', '', parts[1]) if len(parts) > 1 else None
        year = re.sub(r'[^0-9]', '', parts[2]) if len(parts) > 2 else None
        cvv = re.sub(r'[^0-9]', '', parts[3]) if len(parts) > 3 else None
        
        if len(bin_part) < 6:
            bot.reply_to(message, "❌ BIN must be at least 6 digits")
            return
        
        cards = []
        for i in range(10):
            remaining_digits = 16 - len(bin_part)
            if remaining_digits > 0:
                random_part = ''.join([str(random.randint(0, 9)) for _ in range(remaining_digits)])
                card_number = bin_part + random_part
            else:
                card_number = bin_part[:16]
            
            if month and month.isdigit() and 1 <= int(month) <= 12:
                card_month = month.zfill(2)
            else:
                card_month = str(random.randint(1, 12)).zfill(2)
            
            if year and year.isdigit():
                if len(year) == 2:
                    card_year = year
                elif len(year) == 4:
                    card_year = year[2:]
                else:
                    card_year = str(random.randint(23, 30))
            else:
                card_year = str(random.randint(23, 30))
            
            if cvv and cvv.isdigit() and 3 <= len(cvv) <= 4:
                card_cvv = cvv
            else:
                card_cvv = str(random.randint(100, 999))
            
            cards.append(f"{card_number}|{card_month}|{card_year}|{card_cvv}")
        
        # الحصول على معلومات BIN باستخدام الدالة الجديدة
        bin_info = get_bin_info(bin_part)  # هذه تُرجع قاموس
        
        cards_text = '\n'.join(cards)
        
        # بناء الرسالة مع معلومات BIN
        msg = f"""
<strong>𝗖𝗮𝗿𝗱𝘀 𝗚𝗲𝗻𝗲𝗿𝗮𝘁𝗲𝗱 ✅
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐁𝐈𝐍: <code>{bin_part[:6]}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐂𝐨𝐮𝐧𝐭: <code>10</code>
- - - - - - - - - - - - - - - - - - - - - - -
<code>{cards_text}</code>
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐁𝐈𝐧 𝐈𝐧𝐟𝐨: <code>{bin_info['brand']} - {bin_info['card_type']} - {bin_info['level']}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐁𝐚𝐧𝐤: <code>{bin_info['bank']} - {bin_info['country_flag']}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐂𝐨𝐮𝐧𝐭𝐫𝐲: <code>{bin_info['country_name']} [ {bin_info['country_flag']} ]</code>
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">⌤</a>] 𝐀𝐧𝐚𝐥𝐲𝐳𝐞𝐝 𝐛𝐲: <a href='tg://user?id={message.from_user.id}'>𝐴𝐻𝑀𝐸𝐷</a>@A_A_T_T</strong>"""
        
        bot.reply_to(message, msg, parse_mode='HTML')
        
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {str(e)}\nUse: /gen 552742|12|25|123")
# =========================
# Bin Command - Fixed
# =========================
@bot.message_handler(commands=['bin'])
def bin_check(message):
    try:
        if len(message.text.split()) < 2:
            bot.reply_to(message, "⚠️ Usage: /bin BIN_number\nExample: /bin 552742")
            return
        
        bin_number = message.text.split()[1]
        bin_number = re.sub(r'[^0-9]', '', bin_number)
        
        if len(bin_number) < 6:
            bot.reply_to(message, "❌ BIN must be at least 6 digits")
            return
        
        response = requests.get(f'https://lookup.binlist.net/{bin_number[:6]}', timeout=10)
        
        if response.status_code != 200:
            bot.reply_to(message, "❌ Could not find info for this BIN")
            return
        
        data = response.json()
        
        # الحصول على معلومات BIN من API الثاني
        try:
            response2 = requests.get(f"https://bins.antipublic.cc/bins/{bin_number[:6]}", timeout=5)
            if response2.status_code == 200:
                bin_data = response2.json()
                brand = bin_data.get("brand", "Unknown").upper()
                card_type = bin_data.get("type", "Unknown").upper()
                level = bin_data.get("level", "Unknown").upper()
                bank = bin_data.get("bank", "Unknown")
                country_name = bin_data.get("country_name", "Unknown")
                country_flag = bin_data.get("country_flag", "🏳️")
            else:
                raise Exception()
        except:
            brand = data.get('scheme', 'Unknown').upper()
            card_type = data.get('type', 'Unknown').upper()
            level = ""
            bank = data.get('bank', {}).get('name', 'Unknown')
            country_name = data.get('country', {}).get('name', 'Unknown')
            country_flag = data.get('country', {}).get('emoji', '🏳️')
        
        country_code = data.get('country', {}).get('alpha2', 'Unknown')
        prepaid = str(data.get('prepaid', 'Unknown')).upper()
        
        # تنسيق الرسالة بنفس الشكل المطلوب
        msg = f"""
<strong>𝗕𝗜𝗡 𝗟𝗼𝗼𝗸𝘂𝗽 𝗥𝗲𝘀𝘂𝗹𝘁 🔍
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐁𝐈𝐍: <code>{bin_number[:6]}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐈𝐧𝐟𝐨: <code>{brand} - {card_type} - {level}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐁𝐚𝐧𝐤: <code>{bank} - {country_flag}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐂𝐨𝐮𝐧𝐭𝐫𝐲: <code>{country_name} [ {country_flag} ]</code>
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐏𝐫𝐞𝐩𝐚𝐢𝐝: <code>{prepaid}</code>
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐂𝐨𝐮𝐧𝐭𝐫𝐲 𝐂𝐨𝐝𝐞: <code>{country_code}</code>
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">⌤</a>] 𝐀𝐧𝐚𝐥𝐲𝐳𝐞𝐝 𝐛𝐲: <a href='tg://user?id={message.from_user.id}'>𝐴𝐻𝑀𝐸𝐷</a>@A_A_T_T</strong>"""
        
        bot.reply_to(message, msg, parse_mode='HTML')
        
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {str(e)}")

# =========================
# Website Analysis Functions - FIXED VERSION
# =========================

def analyze_site(url):
    """تحليل موقع وإرجاع النتائج"""
    if not url.startswith(('http://', 'https://')):
        url = 'http://' + url
    
    result = {
        'url': url, 
        'payment_gateways': [], 
        'captcha': False, 
        'cloudflare': False, 
        'graphql': False, 
        'platform': None, 
        'http_status': None, 
        'content_type': None, 
        'cookies': {}, 
        'error': None, 
        'country': None
    }

    try:
        response = requests.get(url, timeout=10, headers={
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
        })
        headers = response.headers
        content_type = headers.get('Content-Type', '')
        response_text = response.text
        cookies = response.cookies.get_dict()
        country = headers.get('CF-IPCountry', 'Unknown')

        http_version = 'HTTP/1.1' if response.raw.version == 11 else 'HTTP/1.0'
        status_code = response.status_code
        reason_phrase = response.reason
        http_status = f"{http_version} {status_code} {reason_phrase}"

        result.update({
            'payment_gateways': check_for_payment_gateways(headers, response_text, cookies),
            'cloudflare': check_for_cloudflare(response_text),
            'captcha': check_for_captcha(response_text),
            'graphql': check_for_graphql(response_text),
            'platform': check_for_platform(response_text),
            'http_status': http_status,
            'content_type': content_type,
            'cookies': cookies,
            'country': country
        })

    except requests.Timeout:
        result['error'] = '⏰ Timeout error'
    except Exception as e:
        result['error'] = f'❌ Error: {str(e)}'
    
    return result


def check_for_payment_gateways(headers, response_text, cookies):
    """اكتشاف بوابات الدفع"""
    gateway_keywords = [
        'stripe', 'paypal', 'square', 'venmo', 'bitcoin', 'braintree', 'amazon-pay',
        'adyen', '2checkout', 'skrill', 'authorize.net', 'worldpay', 'payu', 'paytm',
        'afterpay', 'alipay', 'klarna', 'affirm', 'bluesnap', 'checkout.com', 'dwolla',
        'paddle', 'payoneer', 'sagepay', 'wechat pay', 'yandex.money', 'zelle',
        'shopify', 'buy now', 'add to cart', 'store', 'checkout', 'cart', 'shop now',
        'card', 'payment', 'gateway', 'checkout button', 'pay with'
    ]

    combined_text = response_text.lower() + str(headers).lower() + str(cookies).lower()
    detected_gateways = [keyword.capitalize() for keyword in gateway_keywords if keyword in combined_text]

    return list(set(detected_gateways))


def check_for_cloudflare(response_text):
    """التحقق من Cloudflare"""
    cloudflare_markers = ['checking your browser', 'cf-ray', 'cloudflare']
    return any(marker in response_text.lower() for marker in cloudflare_markers)


def check_for_captcha(response_text):
    """التحقق من CAPTCHA"""
    captcha_markers = ['recaptcha', 'g-recaptcha']
    return any(marker in response_text.lower() for marker in captcha_markers)


def check_for_graphql(response_text):
    """التحقق من GraphQL"""
    graphql_markers = ['graphql', 'application/graphql']
    return any(marker in response_text.lower() for marker in graphql_markers)


def check_for_platform(response_text):
    """اكتشاف منصة الموقع"""
    platform_markers = {
        'woocommerce': ['woocommerce', 'wc-cart', 'wc-ajax'],
        'magento': ['magento', 'mageplaza'],
        'shopify': ['shopify', 'myshopify'],
        'prestashop': ['prestashop', 'addons.prestashop'],
        'opencart': ['opencart', 'route=common/home'],
        'bigcommerce': ['bigcommerce', 'stencil'],
        'wordpress': ['wordpress', 'wp-content'],
        'drupal': ['drupal', 'sites/all'],
        'joomla': ['joomla', 'index.php?option=com_']
    }

    for platform, markers in platform_markers.items():
        if any(marker in response_text.lower() for marker in markers):
            return platform.capitalize()

    return None


def format_analysis_results(results, user_id):
    """تنسيق نتائج التحليل"""
    analyzed_by = f"<a href='tg://user?id={user_id}'>𝐴𝐻𝑀𝐸𝐷</a>" if user_id else "𝐴𝐻𝑀𝐸𝐷"
    
    msg = f"""
<strong>𝗦𝗜𝗧𝗘 𝗔𝗡𝗔𝗟𝗬𝗦𝗜𝗦 𝗥𝗘𝗦𝗨𝗟𝗧𝗦 🔍
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐔𝐑𝐋: <code>{results['url']}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐏𝐚𝐲𝐦𝐞𝐧𝐭 𝐆𝐚𝐭𝐞𝐰𝐚𝐲𝐬: <code>{', '.join(results['payment_gateways']) if results['payment_gateways'] else 'None'}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐂𝐀𝐏𝐓𝐂𝐇𝐀: <code>{'✅ Yes' if results['captcha'] else '❌ No'}</code>
[<a href="https://t.me/A_A_T_T">ϟ</a>] 𝐂𝐥𝐨𝐮𝐝𝐟𝐥𝐚𝐫𝐞: <code>{'✅ Yes' if results['cloudflare'] else '❌ No'}</code>
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐆𝐫𝐚𝐩𝐡𝐐𝐋 𝐃𝐞𝐭𝐞𝐜𝐭𝐞𝐝: <code>{'✅ Yes' if results['graphql'] else '❌ No'}</code>
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐏𝐥𝐚𝐭𝐟𝐨𝐫𝐦: <code>{results['platform'] or 'Unknown'}</code>
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐇𝐓𝐓𝐏 𝐒𝐭𝐚𝐭𝐮𝐬: <code>{results['http_status'] or 'Unknown'}</code>
[<a href="https://t.me/A_A_T_T">⌥</a>] 𝐂𝐨𝐮𝐧𝐭𝐫𝐲: <code>{results['country']}</code>
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/A_A_T_T">⌤</a>] 𝐀𝐧𝐚𝐥𝐲𝐳𝐞𝐝 𝐛𝐲: {analyzed_by}</strong>"""
    return msg


# =========================
# URL Analysis Command - FIXED
# =========================
# =========================
# URL Analysis Handler
# =========================
# متغير عام لإيقاف التحليل
stop_analysis = {}

# =========================
# URL Analysis Handler
# =========================
@bot.message_handler(commands=['url'])
def url_analysis(message):
    try:
        if len(message.text.split()) < 2:
            bot.reply_to(message, "Usage: /url <website_url>\nExample: /url https://example.com")
            return
        
        url = message.text.split('/url ', 1)[1].strip()
        
        wait_msg = bot.reply_to(message, "Please wait, analyzing website...")
        
        # تحليل الموقع
        result = analyze_site(url)
        
        # التحقق من وجود أخطاء
        if result['error']:
            bot.delete_message(message.chat.id, wait_msg.message_id)
            bot.reply_to(message, f"Error: {result['error']}")
            return
        
        # تنسيق النتائج
        analysis = format_analysis_results(result, message.from_user.id)
        
        bot.delete_message(message.chat.id, wait_msg.message_id)
        bot.reply_to(message, analysis, parse_mode='HTML')
        
    except Exception as e:
        try:
            bot.delete_message(message.chat.id, wait_msg.message_id)
        except:
            pass
        bot.reply_to(message, f"Error analyzing website: {str(e)}")


# =========================
# Analyze All - Request File
# =========================
@bot.message_handler(commands=['analyze_all'])
def ask_for_file(message):
    """طلب ملف من المستخدم"""
    msg = bot.reply_to(
        message,
        "Please send a .txt file with URLs (one per line)\n"
        "ارسل ملف .txt يحتوي على روابط (رابط في كل سطر)"
    )
    
    # تخزين حالة المستخدم في انتظار الملف
    bot.register_next_step_handler(msg, process_file_and_analyze)


# =========================
# Process File and Analyze
# =========================
def process_file_and_analyze(message):
    """معالجة الملف وبدء التحليل"""
    try:
        # التحقق من أن الرسالة تحتوي على ملف
        if not message.document:
            bot.reply_to(message, "Please send a .txt file | ارسل ملف نصي")
            return
        
        # التحقق من امتداد الملف
        if not message.document.file_name.endswith('.txt'):
            bot.reply_to(message, "Only .txt files accepted | الملفات النصية فقط (.txt)")
            return
        
        # رسالة تحميل
        loading_msg = bot.reply_to(message, "Downloading file... | جاري تحميل الملف...")
        
        # تحميل الملف
        file_info = bot.get_file(message.document.file_id)
        downloaded = bot.download_file(file_info.file_path)
        
        # قراءة المحتوى
        content = downloaded.decode('utf-8', errors='ignore')
        lines = content.splitlines()
        
        # استخراج الروابط
        urls = []
        for line in lines:
            line = line.strip()
            # تجاهل الأسطر الفارغة والتعليقات
            if line and not line.startswith('#'):
                # إضافة http إذا لم يكن موجوداً
                if not line.startswith(('http://', 'https://')):
                    line = 'http://' + line
                urls.append(line)
        
        if not urls:
            bot.edit_message_text(
                "No valid URLs found in file | لا توجد روابط صالحة في الملف",
                message.chat.id,
                loading_msg.message_id
            )
            return
        
        # تحديث الرسالة
        bot.edit_message_text(
            f"File loaded successfully!\n"
            f"Starting analysis... | جاري بدء التحليل...",
            message.chat.id,
            loading_msg.message_id
        )
        
        time.sleep(1)
        
        # بدء التحليل
        analyze_urls_one_by_one(message, urls, loading_msg.message_id)
        
    except Exception as e:
        bot.reply_to(message, f"Error processing file: {str(e)}")


# =========================
# Analyze URLs One by One
# =========================
def analyze_urls_one_by_one(message, urls, progress_msg_id):
    """تحليل الروابط واحداً تلو الآخر"""
    try:
        chat_id = message.chat.id
        total = len(urls)
        successful = 0
        failed = 0
        payment_sites = 0
        
        # إعادة تعيين حالة الإيقاف
        stop_analysis[chat_id] = False
        
        # بدء التحليل
        for i, url in enumerate(urls, 1):
            try:
                # التحقق من طلب الإيقاف
                if stop_analysis.get(chat_id, False):
                    bot.edit_message_text(
                        f"Analysis stopped by user | تم ايقاف التحليل\n\n"
                        f"Progress: {i-1}/{total}\n"
                        f"Successful: {successful}\n"
                        f"Failed: {failed}\n"
                        f"Payment sites: {payment_sites}",
                        chat_id,
                        progress_msg_id
                    )
                    return
                
                # تحديث رسالة التقدم كل 3 روابط
                if i % 3 == 0 or i == 1 or i == total:
                    try:
                        bot.edit_message_text(
                            f"Progress: {i}/{total}\n"
                            f"Successful: {successful}\n"
                            f"Failed: {failed}\n"
                            f"Payment sites: {payment_sites}\n\n"
                            f"Current:\n{url[:50]}{'...' if len(url) > 50 else ''}\n\n"
                            f"Use /stop to stop analysis",
                            chat_id,
                            progress_msg_id
                        )
                    except:
                        pass
                
                # تحليل الموقع
                result = analyze_site(url)
                
                # التحقق من النتائج
                if result.get('error'):
                    failed += 1
                else:
                    successful += 1
                    
                    # إذا وجدت بوابات دفع، أرسل النتائج فقط
                    if result.get('payment_gateways'):
                        payment_sites += 1
                        analysis = format_analysis_results(result, message.from_user.id)
                        
                        # إرسال نتيجة التحليل بدون زيادات
                        bot.send_message(
                            chat_id,
                            analysis,
                            parse_mode='HTML'
                        )
                
                # تأخير بسيط لتجنب الحظر
                time.sleep(0.3)
                
            except Exception as e:
                failed += 1
                print(f"Error analyzing {url}: {str(e)}")
                continue
        
        # حذف رسالة التقدم النهائية
        try:
            bot.delete_message(chat_id, progress_msg_id)
        except:
            pass
        
        # مسح حالة الإيقاف
        if chat_id in stop_analysis:
            del stop_analysis[chat_id]
        
    except Exception as e:
        bot.send_message(chat_id, f"Error during analysis: {str(e)}")


# =========================
# Stop Analysis Command
# =========================
@bot.message_handler(commands=['stop'])
def stop_analysis_command(message):
    """إيقاف التحليل الجاري"""
    chat_id = message.chat.id
    
    if chat_id in stop_analysis and not stop_analysis[chat_id]:
        stop_analysis[chat_id] = True
        bot.reply_to(message, "Stopping analysis... | جاري ايقاف التحليل...")
    else:
        bot.reply_to(message, "No active analysis to stop | لا يوجد تحليل جاري")


# =========================
# Cancel Command
# =========================
@bot.message_handler(commands=['cancel'])
def cancel_operation(message):
    """إلغاء العملية الحالية"""
    bot.reply_to(message, "Operation cancelled | تم الغاء العملية")

# =========================
# Unknown Command Handler
# =========================
@bot.message_handler(func=lambda message: True)
def handle_unknown(message):
    if message.text.startswith('/'):
        bot.reply_to(message, "❌ Unknown command.\n\nAvailable commands:\n/start - Start\n/fake - Fake info\n/gen - Generate cards\n/bin - Check BIN\n/url - Analyze website\n/analyze_all - Analyze all URLs from file")

# =========================
# Run Bot
# =========================
print("✅ Bot is running...")
print("📱 Go to Telegram and search for the bot")
print("⚡ Use /start to begin")

while True:
    try:
        bot.polling(none_stop=True, timeout=30)
    except Exception as e:
        print(f"⚠️ Error: {e}")
        time.sleep(5)